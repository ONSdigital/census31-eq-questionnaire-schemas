...
def update_schemas(schemas_dir):
    language_dirs = filter(
        lambda d: os.path.isdir(f"{schemas_dir}/{d}"), os.listdir(schemas_dir)
    )

    for language_code in language_dirs:
        language_dir = f"{schemas_dir}/{language_code}"
        for schema_name in os.listdir(language_dir):
            ...
            if replaced or removed:
                with open(schema_filepath, "w", encoding="utf-8") as schema_file:
